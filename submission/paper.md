...
This is a semester-scale prototype, not a production service. The controlled scope limits model breadth, retrieval variants, answer-key annotation, independent review, and deployment testing. Results apply to the frozen implementation and samples rather than all users, courts, domains, or changing corpora. The researcher interface comprises a static local frozen-artifact viewer and a connected local React + Spring Boot + FastAPI research demo (loopback-only, not production); latency, memory, and compute efficiency metrics are not reported; and no standalone architecture-diagram file ships beyond the in-text architecture description and results figures.
