"""Run E4 retrieval, verified explanation, and evidence-augmented prediction."""
...
def main() -> None:
    ...
    if answer_config["generation_mode"] != "controlled_extract_only":
        raise ValueError("E4 requires the frozen controlled extract-only renderer")
    ...
    predictor = EvidenceAugmentedPredictor.from_config(args.prediction_config)
    outcome_prediction = predictor.predict(facts_text=args.query, selected_evidence=selected).as_dict()
    ...
    result = {
        ...
        "citation_verification": citation_status,
        "answer": answer,
    ...
    print(json.dumps({
        ...
        "predicted_label": outcome_prediction["predicted_label"],
    }, indent=2))
