...
> Historical evidence-only evaluation record. E3/E4 outcome prediction was added later under `e3e4-evidence-augmented-inlegalbert-checkpoint6318-v1`; see `artifacts/e3_e4_evidence_augmented_evaluation.json`.
...
At the time of this historical run, E3/E4 did not output an outcome label. E3 and E4 shared the frozen retrieval/selection output, and E4 added hard citation, provenance, and temporal verification. The evidence metrics have 30 expected authorities, 150 selected evidence items, and 150 displayed citation checks. The machine-readable evidence-only record is `artifacts/week11_temporal_prerank_evaluation.json`; `artifacts/week11_initial_evaluation.json` is retained as the superseded post-ranking-filter baseline. The later prediction result is stored separately and does not overwrite either artifact.

### Temporal reporting metrics

| Metric | Numerator / denominator | Value |
|---|---|---:|
| Eligible returned candidates | 3,000 / 3,000 | 1.000000 |
| Temporally eligible final cited output | 150 / 150 | 1.000000 |

The final configuration applies strict earlier-year filtering before BM25 ranking and the top-100 cutoff, so every returned candidate and displayed citation is eligible. The preserved post-ranking baseline contained 1,712 later-year candidates among 2,743 logged candidates plus 267 same-year ambiguous candidates; this dilution motivated the bounded test. Two temporal exposure-rate metrics were removed from reporting on the project mentor's guidance, while these direct counts remain.

No E4-minus-E3 outcome comparison was available in this historical evidence-only run. The later revision reports E3 and E4 accuracy and macro F1 separately and does not define a canonical prediction-delta metric. The full temporal definitions and counts are in `artifacts/week11_reporting_framework.md`.
